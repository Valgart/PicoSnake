# main.py
# WLAN-Zugangsdaten aus secrets.py laden
try:
    from secrets import SSID, PASSWORD
except ImportError:
    print('Fehler: Datei secrets.py nicht gefunden! Bitte anlegen.')
    raise
...
# HTML- und JS-Code des Snake-Spiels
html = '''<!DOCTYPE html>
<html lang="de">
<head>
  <meta charset="UTF-8">
...
# Server starten
target=socket.getaddrinfo('0.0.0.0',80)[0][-1]
sock=socket.socket()
sock.bind(target)
sock.listen(1)
ip=connect_wifi(SSID,PASSWORD)
print('Server läuft auf http://%s:80'%ip)
while True:
    client,addr=sock.accept()
    req=client.recv(1024)
    print('Request:',req)
    if b'favicon.ico' in req:
        client.send("HTTP/1.0 404 Not Found\r\nConnection: close\r\n\r\n".encode()); client.close(); continue
    client.send(template_ok.encode())
    data=html.encode('utf-8')
    for i in range(0,len(data),512): client.send(data[i:i+512])
    client.close()
